...
def build_candidate_queue(graph):
	...
	for node in list(graph.nodes(data=True)):
		...
		#print('cnadidate priority queue: ')
		#print(candidate_priority_queue)
		#print('QQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQ')
...
###		Other functions
#####################################################
def tagme_entity_linking(text, tagmeToken):
	tagme_url = "https://tagme.d4science.org/tagme/tag"
	payload = [("gcube-token", tagmeToken),
               ("text", text),
               ("lang", 'en')]
	try:
...
def tagme_get_all_entities(utterance, tagmeToken):
	...
	while not request_successfull:
		try:
			...
			tagme_entity_linking(utterance, tagmeToken)
			# ##################################################################
			tagme_url = "https://tagme.d4science.org/tagme/tag"
...
def answer_complete_question(question, tagmeToken):
	...
	for entity in entities:
		...
		print(statements)
		with open("initial_question_all_statements.json", "w") as f:
		...
		for statement in statements:
			...
			if highest_matching_similarity == -1 or matching_similarity > highest_matching_similarity:
				#print('*****************************************')
				#print('Statement that contain all identifier predicates:')
				#with open("initial_question_identifier_predicates_statements.json", "w") as f:
					#json.dump(statement, f)
				#print(statement)
				#print('*****************************************')
				answer = statement['entity']['id'] if statement['object']['id'] == entity['wikidata_id'] else \
				statement['object']['id']
				context = {'entity': {'id': entity['wikidata_id']}, 'predicate': {'id': statement['predicate']['id']},
						   'object': {'id': answer}}
				result = {'context': context, 'answers': [{'answer': answer, 'rank': 1}]}
				highest_matching_similarity = matching_similarity
	print("111111111111111111111111111111111111111")
	print('Initial answer is:')
...
def answer_follow_up_question(question, turn, graph, hyperparameters, number_of_frontier_nodes):
	...
	ranked_answers = get_top_k_answers_ranked(answers, k=5)
	#print("初始的后续问题top5答案")
	#print(ranked_answers)
	top_1 = get_top_k_answers_ranked(answers, k=1)	
	gp.set_all_nodes_as_qa_nodes(graph)
	
	if question_is_existential(question): #如果是这种疑问句 特殊例子 答案就是固定的 yes 或no
		ranked_answers = [{'answer': "yes", 'answer_score': 1.0, 'rank': 1}, {'answer': "no", 'answer_score': 0.5, 'rank': 2}]
	print("********Final top answers:********")
	print(ranked_answers)
	#print('The final answer list is: [ Dexter@en, 2007-01-11T00:00:00Z, '+wd.wikidata_id_to_label('Q34')+' ]')
...
def answer_conversation(questions, tagmeToken, hyperparameters, number_of_frontier_nodes):
	...
	print('The final answer lists is: ')
	print(answers)
	return answers
